...
public class KartController_Backup : NetworkBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (Input.GetKey(KeyCode.T)) {
            rb.AddForce(transform.forward * 1000f); // Kiểm tra xem có di chuyển không
        }
        ...
        LimitDriftRotation();
        SendPositionToServerRpc(rb.position, rb.rotation);
        
        
        if (IsOwner && !rb.isKinematic && Mathf.Abs(Input.GetAxis("Vertical")) < 0.1f)
        {
            Vector3 horizontalVelocity = new Vector3(rb.velocity.x, 0, rb.velocity.z);
    ...
    public override void OnNetworkSpawn()
    {
        ...
        Debug.Log($"rearLeftWheel.isGrounded = {rearLeftWheel.isGrounded}");
        Debug.Log($"Mass: {rb.mass}, UseGravity: {rb.useGravity}");

        if (IsOwner)
        {
            Debug.Log("📡 Bây giờ mới gửi RequestCarListServerRpc sau khi xe đã được spawn!");
        ...
        Debug.Log($"[Client] LocalClientId = {NetworkManager.Singleton.LocalClientId}, This object's Owner = {OwnerClientId}, IsOwner = {IsOwner}");
    }
    ...
    private IEnumerator SetupRigidbody()
    {
        ...
        if (IsOwner)
        {
            ...
            Debug.Log("✅ [Owner] Rigidbody = dynamic + Interpolate");
        }
        else
        {
            rb.isKinematic = true;
            rb.interpolation = RigidbodyInterpolation.None;
            Debug.Log("⛔ [Non-Owner] Rigidbody = kinematic + no interpolate");
        }
    ...
    private IEnumerator InitializeKart()
    {
        ...
        {
            hasInitialized = true;
            Debug.Log($"✅ [Client] Player {OwnerClientId} đã spawn và thêm vào checkpoint system!");
        }
    }
    ...
    {
        Debug.Log($"[Client] Input: accel={accel}, steer={steer}, brake={isBraking}");
